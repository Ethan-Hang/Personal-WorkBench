...
export const SETTINGS_CODECS = {
  ...
  /**
   * 被关掉的模块 id。**只影响界面**——路由、导航、命令面板与该模块贡献到别处的 UI
   * 都不再出现，但服务端路由照常注册、数据照常在、已投影成 core Item 的东西
   * （招聘的面试轮次）照常出现在日历与今日。关掉不等于卸载。
   *
   * 「核心工作」那组不可关（关了就没有今日与周历，首页重定向也没了目标），
   * 这一条由消费方强制，core 不知道哪些模块算核心。
   */
  'workbench.disabledModules': idList(),
  /**

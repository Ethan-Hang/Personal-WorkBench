...
| 共享设计基座                 | ✅   | `packages/ui` 五个原语 + 「现代温暖」token（自原型移植）。**只是薄薄一层** |
...
| 工作流                 | 归属       | 依赖       | 内容                                                             |
| ---------------------- | ---------- | ---------- | ---------------------------------------------------------------- |
| **主题层**             | 前端开发者 | 无         | 把设计基座补成完整视觉系统；统一现有页面                         |
...
**主题层的设计由前端开发者自己定，不由后端代写。** 现有的 `packages/ui` 只是从原型搬来的
最小一层（5 个原语 + 一组 token），足以让页面不难看，**远不是完整的视觉系统**。「现代温暖」
具体长什么样、组件体系如何划分，需要对着屏幕反复调——隔空写 spec 只会限制他。

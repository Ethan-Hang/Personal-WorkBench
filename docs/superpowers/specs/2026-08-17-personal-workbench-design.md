...
| **工作台模块**         | 后端       | 无         | 服务端 ✅：跨模块今日聚合、排程 API、待排程抽屉。**UI 搬迁待前端做**       |
| **周日历 UI**          | 待定       | 上面两条   | 7 列日网格 + 拖拽排程（**颗粒度 1 分钟**）。开工前先读 ADR-0012 的已知限制 |
...
### 14.4 待办全生命周期闭环、DatePicker 与动效系统规范（最新修复归档）
...
2. **整合型 DatePicker 交互与极简模拟钟表标准（ADR-0013）**：
   - **整合输入框与日历 Popper**：支持键盘 `YYYY / MM / DD  HH : mm` 掩码直输与日历/时钟交互无缝双向同步；
   - **双指针直接抓取拖动（Direct-Grab）**：为时针与分针建立独立透明加大捕获热区，用户可直接按住任意指针拖动旋转，表盘空白处内圈（$r \le 42$）智能分配为时针、外圈（$r > 42$）智能分配为分针，消除预选繁琐与误触跳变；
   - **针感聚焦与微透高亮**：输入小时时时针高亮变色，输入分钟时分针高亮变色；时分数字选中态采用浅色微透底（`bg-accent-soft`）与轮廓光（`ring-1 ring-accent/30`），摒弃沉重实底；
   - **时分自动感知与双制式支持**：输入时分自动包含时间，点击「仅保留日期」自动回退纯日期；支持 24 小时制与带 `AM` / `PM`（自动大写转换）的 12 小时制；
   - **场景化 `showTime` 模式约束**：排程与投递截止日等纯日期字段设置 `showTime={false}`，完全隐藏时钟与时分输入并收敛为紧凑单列日历（`w-72`）；面试时间与死线等设置 `showTime={true}` 展开日历与钟表双栏；
   - **即时错误标红警示**：遇到输入未完成或年份/月份/天数/时分越界时，输入框即时呈现红色警戒边框与柔和光晕（`border-critical ring-2 ring-critical/20`）并弹出精准错误微标。

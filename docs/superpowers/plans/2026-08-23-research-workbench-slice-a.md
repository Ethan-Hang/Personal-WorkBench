...
- [ ] 在 `modules/research/package.json` 声明 server、contract、storage、ui 和 ui/api 导出，锁定 `pdfjs-dist@6.2.108` 与 `fast-xml-parser@5.11.0`。
- [ ] 在 `contract.ts` 定义版本化 `/api/research/v1` 路径、ID、分页、Work/Edition/Asset/Location/Attachment、字段来源、导入草稿、文件状态和机器可读错误 Zod schema。
- [ ] 为 managed/link、导入阶段、位置状态、元数据来源、重复候选决策和删除动作使用显式枚举，不把语义藏进自由字符串。
- [ ] 编写契约测试：最小 Work、unknown 类型、不完整字段、带多来源建议、缺失位置和失败导入均可解析；非法状态组合被拒绝。
- [ ] 创建 A1/A2 共用表和索引；迁移账本只属于 research 模块。
- [ ] 为 schema 添加真实 SQLite 迁移测试，检查外键、唯一 hash、允许重复 DOI、多目录关系和账号库重复迁移幂等。
- [ ] 在边界测试中确认 research 的非 storage 代码不能 import 数据库库或 `@workbench/data`。
- [ ] 更新 workspace lockfile、TypeScript paths 和必要 ESLint 配置。
...
- [ ] Repository 接口按领域操作组织，不向 service 暴露 SQL 行或通用 CRUD。
- [ ] 实现 Work/Edition/Contributor/Identifier、Asset/Location/Attachment、Collection/Entry、SourceRecord/Assertion、ImportSession/Item 的原子写入与查询。
- [ ] 使用稳定 ID（UUID）并让所有时间由可注入 clock 生成，测试不依赖真实时间。
- [ ] hash 唯一竞争由数据库约束兜底；重复标识符只返回候选，不抛唯一冲突。
- [ ] 列表使用 keyset 分页；批量加载避免逐行 N+1。
- [ ] 生成 10,000 Work、20,000 Edition/Asset/Location/Attachment、20,000 CollectionEntry、1,000 Tag 和 30,000 WorkTag 的临时磁盘基准。
- [ ] 记录数据库大小、峰值 RSS、普通查询 p95、缺失/重复/对账 p95、200 条事务、1,000 次 hash 查询和 `integrity_check`。
- [ ] 默认规模测试不创建等量 PDF，临时目录退出时清理；超阈值时测试明确失败并打印实测值。
...
- [ ] 定义窄 `FileSystem` 边界，生产适配器使用 Node `fs`，测试适配器可注入取消、锁定、权限、空间不足和跨设备错误。
- [ ] 实现分块 SHA-256、进度、AbortSignal 和大小检查，不把整份 PDF 读入内存。
- [ ] 托管导入在 managed root 内创建 staging 文件，完成关闭与校验后原子 rename；并发导入只留一个正式对象。
- [ ] 链接导入保存原路径、realpath、大小、mtime 和可选 dev/ino；不复制、不改写源文件。
- [ ] 路径选择器支持 macOS `osascript`、Windows PowerShell 和 Linux zenity/kdialog 的命令适配，取消统一返回 `null`；选择后仍由服务端重新校验 PDF 路径。
- [ ] 上传入口只接受 managed 模式，流式写入当前账号 staging；linked 模式拒绝网页上传产生的临时路径。
- [ ] 实现缺失审计、同 hash 重新定位、不同 hash replacement candidate、附件回收和无引用托管对象永久清理前二次确认。
- [ ] 实现启动/手动 reconciliation：处理 staging 未提交、对象已就位但数据库未提交、数据库显示可用但文件缺失或变化。
- [ ] 所有错误返回稳定 code、阶段、可重试性和安全清理结果。
- [ ] 用 30–50 个小型生成 PDF/二进制样本覆盖正常、Unicode、符号链接、重复、并发、缺失、只读、取消和三处中断点。
- [ ] 更新 `scripts/research-windows-file-semantics.mjs` 以覆盖实际 content-store 行为；在兼容性记录中保留 Windows 待测状态和运行指令。
...
- [ ] `pdf-worker.mjs` 只加载 `pdfjs-dist/legacy`，读取嵌入元数据和第一页文本，不渲染页面、不建全文索引。
- [ ] `pdf-extractor.ts` 以独立子进程运行 worker，设置超时、输出上限和强制终止；解析崩溃不得带倒本地 server。
- [ ] 从嵌入字段、首页文本和文件名提取标题、作者、年份、DOI、arXiv ID 候选，并保留来源及观察时间。
- [ ] 标识符规范化单独测试：DOI URL/prefix、arXiv 新旧格式、大小写、尾随标点和无效输入。
- [ ] 生成最小 PDF fixtures：内嵌元数据、首页 DOI、首页 arXiv、扫描型无文本、损坏/截断、混乱文件名和不完整字段。
- [ ] 解析失败只产生明确警告和人工确认入口，不回滚已经安全完成的 hash/存储阶段。
- [ ] 256 MiB 大文件测试运行时生成并由 `RUN_RESEARCH_LARGE_FILE=1` 启用，验证流式 hash、取消和临时文件清理。
...
- [ ] 定义统一 `MetadataProvider` 和可注入 `HttpClient`，生产代码使用 Node fetch，单元测试使用固定响应。
- [ ] arXiv Atom 用 `fast-xml-parser` 解析并关闭实体处理。
- [ ] DOI 流程先查 Crossref agency，再向 Crossref 或 DataCite 精确接口查询；OpenAlex DOI 精确结果只作补充候选。
- [ ] 无可靠 ID 时，OpenAlex 标题/作者/年份和 Crossref bibliographic 只返回候选，不自动认定作品。
- [ ] 实现每提供方限速、并发 1、8 秒超时、最多 2 次网络/429/5xx 重试并遵守 `Retry-After`。
- [ ] 缓存 exact success；not-found 24 小时；transient failure 5 分钟。普通 4xx 不重试。
- [ ] 持久化原始响应、解析版本、请求关键字和获取时间；不保存密钥，不发送 PDF。
- [ ] 字段决策顺序为人工确认 > 精确外部 > 其他外部 > PDF embedded > 首页 > 文件名；同级冲突保留全部候选。
- [ ] 用户确认写入新的人工 assertion 并选择为 current；刷新外部结果不能覆盖人工字段。
- [ ] 第一次启用外部查询时通过 UI/API 返回将访问的服务及会发送的字段，用户可保持离线导入。
- [ ] 固定响应覆盖成功、无结果、超时、429、500、无效 XML/JSON、重复刷新和人工字段保护。
- [ ] 提供 opt-in 脚本验证四个真实服务，只使用公开标识符样本并打印服务、状态和时间，不进入默认 `npm run check`。
...
- [ ] 导入分为 `prepare → inspect → resolve duplicate → confirm → commit`，未确认前不创建正式 Work。
- [ ] prepare 创建 ImportSession/Item；managed/link 进入相应文件阶段；inspect 并行组合本地与外部候选。
- [ ] confirm 接受用户逐字段选择、存储模式、作品归属、版本决策和目录列表，在一个数据库事务中建立或复用身份关系。
- [ ] 同 hash 且已有明确归属时复用 Asset/Edition/Attachment；不同 hash 同 DOI 必须返回候选选择。
- [ ] 提供最小文献库 API：列表/详情、目录创建与多目录归属、位置检查、重新定位、移除目录引用、移除附件、Work 回收/恢复/永久删除、对账。
- [ ] 永久删除返回影响预览并要求确认 token；执行前在新事务中重算引用，条件变化时拒绝。
- [ ] 所有 mutation 支持幂等 request ID；并发导入和重复提交返回同一可追踪结果。
- [ ] 在 server 组合根注入动态 Repository、accountId、managedRoot 和 file picker，注册 research 迁移与路由。
- [ ] 账号切换测试确认数据库与托管根一起切换，不跨账号复用 Asset 或引用计数。
- [ ] 路由 contract tests 覆盖 A1-01 至 A1-11 的 API 可观察结果。
...
- [ ] 在开始 UI 工作前读取并遵循仓库当前 frontend skill；保持现有 AppShell、主题、间距和组件语言。
- [ ] 注册 `/research` 页面和导航项“文献库”。
- [ ] 导入弹窗支持系统选文件、手工路径和 managed 上传；在确认前选择托管/链接模式。
- [ ] 展示 hash/大小、PDF 本地识别、外部候选、字段来源和冲突；每个字段可人工修正或选择来源。
- [ ] 重复候选明确提供现有版本、新版本、新作品、暂留和放弃，不放置自动合并捷径。
- [ ] 文献库最小视图展示标题、作者/年份、目录、附件模式和 available/missing/changed/recycled 状态。
- [ ] 目录侧栏支持创建目录、加入多个目录和只移除当前目录引用。
- [ ] 缺失位置提供检查与重新定位；不同 hash 显示替换候选，不把按钮写成“恢复成功”。
- [ ] 删除 UI 把移除目录引用、移除附件、Work 回收、永久删除分成四个动作，并展示实际影响。
- [ ] API/UI 状态覆盖离线元数据、解析失败、导入取消、并发复用和部分恢复。
- [ ] 纯逻辑与 API client 使用 Vitest；`.tsx` 测试若要纳入自动套件，先显式修正 `vitest.config.ts` include 并验证不会漏收现有测试。
...
- [ ] 将 A1-01 至 A1-11 映射到自动测试名称，生成一张通过/失败/手动待测矩阵。
- [ ] 默认套件临时空间不超过 250 MiB；专项 200 文件和 opt-in 256 MiB 测试单独运行并清理。
- [ ] 重跑目标规模基准，记录实际表结构的指标，不沿用技术验证原型数字。
- [ ] 执行 `PRAGMA integrity_check`，检查 managed staging、对象目录和数据库引用没有孤儿。
- [ ] 记录当前文件语义测试模块的 macOS 环境与结果，保留 Windows 未测及 PowerShell 指令。
- [ ] 执行 `git fetch origin --prune`，检查 `origin/main` 和远端 feature 状态；不 push。
- [ ] 在本文件勾选 A1 完成项，并依据实际文件和接口校准下面 A2 的逐文件步骤；只修正实施细节，不改变已确认的 A2 范围。
- [ ] 运行 `npm run check`，确认工作区仅含预期修改后提交 A1 验收记录。

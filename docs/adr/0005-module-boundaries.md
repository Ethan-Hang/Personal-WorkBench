...
铁律 1 与铁律 2 由 `eslint.config.js` 的 `no-restricted-imports` 规则强制
（§4.3 的强制表），违反即 CI 失败。`ModuleContext` 不暴露数据库句柄；service/routes
只能拿到 core 能力与模块自有 Repository。

首个自有表模块暴露了一个原设计遗漏：迁移能建表，但模块必须有地方实现这些表的读写。
因此 ADR-0008 增加一个窄例外：模块 storage 目录内的 SQLite 适配器可由组合根注入共享连接，
但不得 import `@workbench/data`，连接也不得继续传入 service/routes。该适配器是可信基础设施
边界；除此以外，“模块够不着数据库句柄”仍成立。
...
- 自有表模块的 storage 适配器是边界中的可信小孔；其文件范围必须保持窄，并由 lint
  限制 SQLite/Drizzle 依赖不得扩散到模块业务代码
